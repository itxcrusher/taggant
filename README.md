...
The camera is asked for in every engine too, and no engine is asked for a real one. `navigator.mediaDevices` is replaced before any page script runs, with a camera that returns a canvas the artwork has been drawn into, so there is no path from the test suite to hardware in any configuration. Whether a canvas can be a camera at all is measured in each engine rather than written down per engine, because it belongs to somebody else's build and differs between platforms. **On the Linux runner the gate runs on, all three engines take the whole path and place the content on the artwork**, each reporting it within eight pixels of where the frame put it and each doing the recognition off the page's thread. On Windows, Playwright's WebKit build has neither `MediaStream` nor a canvas capture, so there the same test checks the other branch: an engine that cannot be given a camera has to report the one it cannot open as an error state the page can show, rather than sitting on Starting for ever. Both branches are strict, and Chromium is required to take the full one everywhere, so the check cannot quietly become nothing.

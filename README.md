...
The camera is asked for in every engine too, and no engine is asked for a real one. `navigator.mediaDevices` is replaced before any page script runs, with a camera that returns a canvas the artwork has been drawn into, so there is no path from the test suite to hardware in any configuration. Chromium and Firefox take that stream through the whole path and place the content on the artwork. Playwright's WebKit has no `MediaStream` at all, measured rather than assumed, so what is checked there is that the runtime reports a camera it cannot open as an error state the page can show, rather than sitting on Starting for ever. The separate Chromium test that plays a video file as a camera is still the most realistic of these, and it is the one that measures frame pacing.

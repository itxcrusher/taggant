...
**One console at a time per workspace and per link table.** Every write it makes is written beside the file and renamed into place, and writes to one experience, one table or one destination are queued behind each other, so there is no window in which a file is half written. None of that survives a second console in another process: both read the file, both decide what the next version of it is, and one of the two edits is gone with both operators told it was saved. So a console takes a lock when it starts, one in the workspace and one beside the link table, and refuses to open either one that a running console already holds, naming the process that holds it. A lock left behind by a crash on this machine is taken over and the takeover is printed. A lock taken on another machine or in a container that is gone cannot be checked from here, so that one is refused and the message names the file to delete.

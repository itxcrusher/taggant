...
What each engine covers is not the same, and pretending otherwise would be the point of this section. Recognition happens in a module worker, and where a browser will not give one the runtime answers on the page's thread instead and says nothing about it, which on a phone would show up as the page freezing rather than as an error. So the worker is started in Chromium, Firefox and WebKit the way the runtime starts it, resolved against a module's own URL rather than a path the test knows, and handed a real target and a frame the artwork is in. Each has to answer with a pose, and all three answer with the same one. Safari was the reason to doubt this, module workers having arrived there late, and it holds. Recognising artwork from a camera feed is Chromium only, because Chromium is the only engine that can be handed a specific video file to play as one. Firefox produces a real camera from a synthetic device, so what can be checked there is the rest of that path: a stream is obtained, the worker the runtime resolves for itself starts, the loop runs, and a feed with nothing recognisable in it leaves the page looking rather than erroring, which is what a viewer pointing a phone at the wrong thing gets. Playwright's WebKit exposes no `navigator.mediaDevices.getUserMedia` at all, measured rather than assumed, so the camera cannot be driven there by anything.

...
Every other asset is identified from its own bytes rather than its name, and passes through only if its signature is one of the image, video or audio formats the bundler publishes. HTML under any extension, UTF-16 text, and anything unrecognised is refused by name. A bundle that asks somebody else's host for part of itself is not a bundle that outlives the service it came from, which is the whole claim, and a test feeds every payload those reviews produced through a real publish and requires that nothing a browser would treat as a document reaches the folder.

...
The number the report cannot yet measure is the field of view. Minimum print width assumes 60 degrees across the picture; a narrower lens makes every minimum smaller and a wider one makes them larger. A phone does not have one field, it has a long axis and a short one, and which is across the picture depends on how the phone is held: measured against this repository's own artwork, a 148 mm print is readable to 185 mm with the phone upright and 140 mm with it turned sideways, against the 192 mm this constant implies. So the assumption is close for a phone held upright, which is how a pack gets scanned, and about a third optimistic for one held sideways. It is marked as an assumption in the source, with those measurements beside it, and <https://itxcrusher.github.io/taggant/measure/> is the page that replaces it with a measurement from a real device.

...
The camera is asked for in every engine too, and no engine is asked for a real one. `navigator.mediaDevices` is replaced before any page script runs, with a camera that returns a canvas the artwork has been drawn into, so there is no path from the test suite to hardware in any configuration. Whether a canvas can be a camera at all is measured in each engine rather than written down per engine, because it belongs to somebody else's build and differs between platforms: on the machine this was written on, Playwright's WebKit has neither `MediaStream` nor a canvas capture, and both branches are strict about what follows. Chromium and Firefox take the stream through the whole path and place the content on the artwork. An engine that cannot be given one has to report a camera it cannot open as an error state the page can show, rather than sitting on Starting for ever, which is what WebKit does here. The separate Chromium test that plays a video file as a camera is still the most realistic of these, and it is the one that measures frame pacing.
